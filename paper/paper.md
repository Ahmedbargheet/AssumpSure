...
*AssumpSure* provides example datasets for all modules. infants.csv is downloadable within the app (Help tab), and the remaining datasets are available in the GitHub repository (inst/extdata/) for manual testing.

- Continuous data tests: t-test.csv, plantgrowth.csv, and infants.csv (for independent/paired t-tests, one-way ANOVA, and other continuous analyses).
- Fisher and Chi-square tests: chi_fisher.csv and infants.csv for categorical comparisons.
- Regression models: infants.csv for linear, logistic, and other regression analyses.
- Correlation analyses: correlation.csv and bacteria_for_correlation.csv for standard and compositional correlation methods.
